...
class res_users(models.Model):
	...
	branch_ids = fields.Many2many('multi.branch', id1='user_id', id2='branch_id', string='Allowed Branches')
...
class pos_session(models.Model):
	...
	branch_id = fields.Many2one('multi.branch',readonly=True)
...
class pos_config(models.Model):
	...
	@api.model
	def create(self, vals):
		result = super(pos_config, self).create(vals)
		if result and result.module_pos_hr:
			if len(result.employee_ids) > 1:
				raise UserError(_("There is only employee allowed per POS session."))
		return result

	def write(self, values):
		result = super(pos_config, self).write(values)
		for pos in self:
			if pos and pos.module_pos_hr:
				if len(pos.employee_ids) > 1:
					raise UserError(_("There is only employee allowed per POS session."))
		return result
...
class POSOrder(models.Model):
	...
	branch_id = fields.Many2one('multi.branch', 'Branch',readonly=True)
...
class pos_payment(models.Model):
	...
	branch_id = fields.Many2one('multi.branch',related='pos_order_id.branch_id')
...
class account_bank_statement(models.Model):
	...
	branch_id = fields.Many2one('multi.branch', 'Branch',related='pos_session_id.branch_id')
...
class account_bank_statement_line(models.Model):
	...
	branch_id = fields.Many2one('multi.branch', 'Branch',related='statement_id.branch_id')
